...
void ExamplePacketListener::ProcessMessage( const osc::ReceivedMessage& m, const IpEndpointName& remoteEndpoint )
{
    try{
		...
		if( strcmp( m.AddressPattern(), "/up") == 0 )	// this is a packet of player control data. /play ID x x x x x ( x = the 5 stream inputs)
		{
            for (int i = 0; i < 5; i++)
                d.data.push_back((arg++)->AsFloat());	// the observed state - 3 values for the moment, will be 15
			d.header = oscUpdate;
        } 
